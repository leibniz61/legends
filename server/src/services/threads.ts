...
import type { Profile, Thread, ThreadDetailResponse, ThreadListResponse } from '@bookoflegends/shared';
...
async function getPostReactions(
  ...
  const userReactions: Record<string, Array<{ id: string; reaction_type: string }>> = {};
  if (userId) {
    ...
    reactions?.forEach((r) => {
      ...
      userReactions[r.post_id].push({ id: r.id, reaction_type: r.reaction_type });
    });
  }
...
export async function getCategoryThreads(
  ...
  const { data: threads, count } = await supabaseAdmin
    .from('threads')
    .select('*, author:profiles!threads_author_id_fkey(id, username, display_name, avatar_url)', {
      count: 'exact',
    })
  ...
  return { threads: threadsWithReadStatus as unknown as ThreadListResponse['threads'], total: count || 0, page };
}
...
async function attachReadStatus(
  ...
  const readMap = new Map(reads?.map((r) => [r.thread_id, r.last_post_number]) || []);
 ...
 */
export async function createThread(options: CreateThreadOptions): Promise<Thread> {
